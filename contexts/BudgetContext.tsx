...
export const [BudgetProvider, useBudget] = createContextHook(() => {
  ...
  const getExpensePaymentStatus = useCallback((expenseId: string, monthYear?: string) => {
    ...
    const isExpenseCheckedInAnyPaycheck = paychecksForMonth.some(p => {
      const checked = (p as any).checkedExpenses || {};
      return checked[expenseId] === true;
    });
    
    if (!isExpenseCheckedInAnyPaycheck) {
      return { status: 'unpaid' as const, amountPaid: 0, amountDue: expense.amount, percentPaid: 0 };
    }
  ...
  const getBillsSummary = useCallback((monthYear?: string) => {
    ...
    if (paychecksForMonth.length === 0) {
      return {
        totalBills,
        totalPaid: 0,
        totalRemaining: totalBills,
      };
    }
    
    const totalPaid = payments
      .filter(p => {
        if (p.monthYear !== targetMonthYear) return false;
        
        const isCheckedInAnyPaycheck = paychecksForMonth.some(paycheck => {
          const checked = (paycheck as any).checkedExpenses || {};
          return checked[p.expenseId] === true;
        });
        
        return isCheckedInAnyPaycheck;
      })
      .reduce((sum, p) => sum + p.amount, 0);
